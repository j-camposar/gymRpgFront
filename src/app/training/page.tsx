...
// API & Types
import { registerDescanso, createTraining, finishWorkOut, getSessionLogs } from '@/services/trainin.api';
...
export default function TrainingPage() {
    ...
    // Fetch de logs de la sesión activa
    useEffect(() => {
    ...
    const handleFinishTraining = () => setShowFinishConfirm(true);
    ...
    return (
        <main className="h-screen bg-[#060606] text-white flex relative overflow-hidden font-sans">
            ...
            {/* --- BARRA DE CONTROL SUPERIOR IZQUIERDA (DATA) --- */}
            <div className="fixed left-6 top-6 z-50 flex flex-row gap-4">
                ...
                <button 
                    ...
                    className="w-14 h-14 rounded-full flex items-center justify-center border-2 transition-all duration-300 shadow-[0_0_15px_rgba(168,85,247,0.3)] bg-purple-600 border-purple-400 hover:scale-110 active:-rotate-12"
                >
                    <span className="text-xl">📋</span>
                </button>
            </div>

            {/* --- BARRA DE CONTROL SUPERIOR DERECHA (ACCIÓN) --- */}
            <div className="fixed right-6 top-6 z-50 flex flex-row-reverse gap-4">
                {/* Burbuja Misiones */}
                <button 
                ...
                {/* <button 
                    ...
                    className="w-14 h-14 rounded-full flex items-center justify-center border-2 transition-all duration-300 shadow-[0_0_15px_rgba(34,197,94,0.3)] bg-green-600 border-green-400 hover:scale-110 active:rotate-12"
                >
                    <span className="text-xl">🍎</span>
                </button> */}
            </div>
            ...
            <aside className={`fixed left-0 top-0 h-full z-40 bg-[#0d0d0d]/98 backdrop-blur-2xl transition-all duration-500 ease-in-out
                ${isStatsOpen ? 'w-full md:w-[450px] translate-x-0 border-r border-blue-500/30' : 'w-0 -translate-x-full border-none'}`}>
            ...
            <aside className={`fixed right-0 top-0 h-full z-40 bg-[#0d0d0d]/98 backdrop-blur-2xl transition-all duration-500 ease-in-out
                ${isMissionsOpen ? 'w-full md:w-[450px] translate-x-0 border-l border-yellow-500/30' : 'w-0 translate-x-full border-none'}`}>
                <div className={`${isMissionsOpen ? 'opacity-100' : 'opacity-0'} transition-opacity duration-300 p-8 pt-24 h-full overflow-y-auto custom-scrollbar`}>
